// FORMAL-ONLY observation ports (`ifdef FORMAL). Non-FORMAL elaboration matches frozen production RTL.
...
`ifdef FORMAL
    ...
    output wire                     f_serve_cpu,
    output wire                     f_target_issued,
    output wire [`ADDR_WIDTH-1:0]   f_a_addr,
    output wire                     f_a_write,
    output wire [`DATA_WIDTH-1:0]   f_a_wdata
`endif
);
...
`ifdef FORMAL
    ...
    assign f_serve_cpu = serve_cpu;
    assign f_target_issued = target_issued;
    assign f_a_addr = a_addr;
    assign f_a_write = a_write;
    assign f_a_wdata = a_wdata;
`endif
