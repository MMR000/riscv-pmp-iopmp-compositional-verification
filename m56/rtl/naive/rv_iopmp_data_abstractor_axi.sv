// M5.6 naive fix overlay — gate W at AXI_HANDSHAKE only (insufficient alone).
// Copied from upstream a029581 with minimal W gating change.
...
typedef enum logic [1:0] {
    IDLE, VERIFICATION, WAIT_IOPMP, AXI_HANDSHAKE
} state_t;
...
assign write_hs_active = (state_q == AXI_HANDSHAKE) && aw_request_q;
assign read_hs_active  = (state_q == AXI_HANDSHAKE) && ar_request_q;

assign axi_aux_req.aw_valid = write_hs_active && slv_req_i.aw_valid;
...
// NAIVE FIX: gate W like AW timing only
assign axi_aux_req.w_valid = write_hs_active && slv_req_i.w_valid;
    ...
    unique case (state_q)
        ...
        AXI_HANDSHAKE: begin
            if (aw_request_q) begin
                if (slv_rsp_o.aw_ready && slv_rsp_o.w_ready)
                    state_n = IDLE;
            end else if (ar_request_q) begin
                if (slv_rsp_o.ar_ready)
                    state_n = IDLE;
            end
        end
        default: ;
