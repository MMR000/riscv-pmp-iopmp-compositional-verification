...
`ifdef FORMAL
  // M5.8: Yosys/read_slang-compatible safety (M57-FP-04 primary target)
  always @(posedge clk) begin
    if (rst_n) begin
      ...
      // M57-FP-04: master W handshake requires authorized write context
      if (mem_write_event && !w_context_ok) assert(0);
      // M57-FP-03 companion: denied context must not produce master W
      if (mem_write_event && txn_active && auth_seq == txn_seq && !txn_authorized)
        assert(0);
    end
  end

  // Cover points for reachability (M5.8 COV)
  always @(posedge clk) begin
    if (rst_n) begin
      if (mem_write_event && w_context_ok) cover(1);
      if (mem_write_event && !w_context_ok) cover(2);
      if (src_aw_hs && env_allow) cover(3);
      if (src_aw_hs && !env_allow) cover(4);
