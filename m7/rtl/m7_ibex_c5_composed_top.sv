// M7 Phase C.5: real Ibex + DMA + IOPMP with independent reset domains (MEM-RET).
...
module m7_ibex_c5_composed_top #(
  ...
  simulator_ctrl #(.LogName("m7_ibex_c5_composed.log")) u_sim_ctrl (
      .clk_i(clk_i), .rst_ni(rst_ni),
  ...
  // Delayed prot path (arbiter <-> delay <-> sram)
  logic arb_prot_req, arb_prot_write, arb_prot_gnt, arb_prot_valid;
  logic [`ADDR_WIDTH-1:0] arb_prot_addr;
  logic [`DATA_WIDTH-1:0] arb_prot_wdata, arb_prot_rdata;
  logic sram_prot_req, sram_prot_write, sram_prot_gnt, sram_prot_valid;
  logic [`ADDR_WIDTH-1:0] sram_prot_addr;
  logic [`DATA_WIDTH-1:0] sram_prot_wdata, sram_prot_rdata;
  logic [7:0] target_delay_cycles;
  logic delay_busy, delay_accepted;
  logic [7:0] delay_count_o;
  ...
  m7_research_arbiter u_ic (
      ...
      .prot_req(arb_prot_req), .prot_addr(arb_prot_addr), .prot_write(arb_prot_write),
      .prot_wdata(arb_prot_wdata), .prot_gnt(arb_prot_gnt), .prot_valid(arb_prot_valid),
      .prot_rdata(arb_prot_rdata)
  );
  ...
  // C.5 instrumentation: optional target delay on protected path only
  m7_target_delay u_delay (
      .clk(clk_i), .rst_n(interconnect_reset_n),
      .delay_cycles(target_delay_cycles),
      .in_req(arb_prot_req), .in_write(arb_prot_write),
      .in_addr(arb_prot_addr), .in_wdata(arb_prot_wdata),
      .in_gnt(arb_prot_gnt), .in_valid(arb_prot_valid), .in_rdata(arb_prot_rdata),
      .out_req(sram_prot_req), .out_write(sram_prot_write),
      .out_addr(sram_prot_addr), .out_wdata(sram_prot_wdata),
      .out_gnt(sram_prot_gnt), .out_valid(sram_prot_valid), .out_rdata(sram_prot_rdata),
      .delay_busy(delay_busy), .delay_count_o(delay_count_o),
  ...
  sram #(.DEPTH(16384)) u_prot_sram (
      ...
      .req(sram_prot_req), .write(sram_prot_write), .addr(sram_prot_addr),
      .wdata(sram_prot_wdata),
      .gnt(sram_prot_gnt), .valid(sram_prot_valid), .rdata(sram_prot_rdata),
  ...
  // Approximate arbiter "serving CPU" for commit attribution
  logic arb_serve_cpu;
  always_ff @(posedge clk_i or negedge interconnect_reset_n) begin
    if (!interconnect_reset_n)
      arb_serve_cpu <= 1'b0;
    else if (cpu_ic_gnt)
      arb_serve_cpu <= 1'b1;
    else if (iopmp_gnt)
  ...
  logic [31:0] cycle_o;
  logic [31:0] stamp_cpu_rel, stamp_dma_rel, stamp_iopmp_rel, stamp_sec_rel, stamp_ic_rel;
  logic [31:0] stamp_pmp_ready, stamp_iopmp_ready, stamp_secure_ready;
  logic [31:0] stamp_dma_req, stamp_dma_admit, stamp_dma_commit, stamp_dma_done;
  logic [31:0] stamp_reset_pulse, stamp_cpu_reset_assert;

  assign harness_dma_cycle_i = stamp_dma_req;
  assign harness_cpu_cycle_i = stamp_cpu_rel;

  m7_c5_harness u_harness (
      ...
      .target_delay_cycles(target_delay_cycles),
      .cfg_write(cfg_write), .cfg_addr(cfg_addr), .cfg_wdata(cfg_wdata),
      ...
      .iopmp_txn_valid(iopmp_txn_valid), .iopmp_txn_auth(iopmp_txn_authorized),
      .delay_busy(delay_busy),
      .delay_accepted(delay_accepted),
      .cpu_adapt_req(cpu_adapt_req),
      .cpu_adapt_addr(cpu_adapt_addr),
      .cpu_adapt_write(cpu_adapt_write),
      .harness_go_set(harness_go_set),
      .harness_result(harness_result_i),
      .cycle_o(cycle_o),
      .sys_secure_ready(sys_secure_ready),
      .test_finished(test_finished),
      .stamp_cpu_rel(stamp_cpu_rel), .stamp_dma_rel(stamp_dma_rel),
      .stamp_iopmp_rel(stamp_iopmp_rel), .stamp_sec_rel(stamp_sec_rel),
      .stamp_ic_rel(stamp_ic_rel),
      .stamp_pmp_ready(stamp_pmp_ready), .stamp_iopmp_ready(stamp_iopmp_ready),
      .stamp_secure_ready(stamp_secure_ready),
      .stamp_dma_req(stamp_dma_req), .stamp_dma_admit(stamp_dma_admit),
      .stamp_dma_commit(stamp_dma_commit), .stamp_dma_done(stamp_dma_done),
      .stamp_reset_pulse(stamp_reset_pulse),
      .stamp_cpu_reset_assert(stamp_cpu_reset_assert)
  );

  m7_c5_event_monitor u_mon (
      .clk(clk_i), .rst_n(rst_ni), .cycle(cycle_o),
      .dma_req(dma_m_req), .dma_gnt_iopmp(dma_m_gnt),
      .dma_valid_iopmp(dma_m_valid), .dma_error_iopmp(dma_m_error),
      .iopmp_txn_valid(iopmp_txn_valid), .iopmp_txn_auth(iopmp_txn_authorized),
      .iopmp_txn_rid(iopmp_txn_rid),
      .iopmp_req_out(iopmp_req), .arb_dma_gnt(iopmp_gnt), .arb_dma_valid(iopmp_valid),
      .dma_addr(dma_m_addr), .dma_wdata(dma_m_wdata), .dma_write(dma_m_write),
      .cpu_adapt_req(cpu_adapt_req), .cpu_adapt_gnt(cpu_adapt_gnt),
      .cpu_adapt_valid(cpu_adapt_valid), .cpu_adapt_write(cpu_adapt_write),
      .cpu_adapt_addr(cpu_adapt_addr), .cpu_adapt_wdata(cpu_adapt_wdata),
      .cpu_reset_n(cpu_reset_n), .dma_reset_n(dma_reset_n),
      .iopmp_reset_n(iopmp_reset_n),
      .security_config_reset_n(security_config_reset_n),
      .interconnect_reset_n(interconnect_reset_n),
      .prot_mem_changed(prot_mem_changed),
      .prot_changed_addr(prot_changed_addr),
      .prot_changed_wdata(prot_changed_wdata),
      .arb_serve_cpu(arb_serve_cpu),
      .delay_accepted(delay_accepted), .delay_busy(delay_busy),
      .pmp_ready(pmp_ready_o), .iopmp_enable(iopmp_enable), .rule0_valid(rule0_valid),
      .sys_secure_ready(sys_secure_ready)
  );
