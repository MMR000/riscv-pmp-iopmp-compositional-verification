// M7 composed SoC: real Ibex (internal PMP) + DMA + IOPMP + shared SRAM.
...
module m7_ibex_composed_top #(
  ...
  logic research_sel;
  logic research_gnt, research_valid;
  logic [31:0] research_rdata;
  logic research_req, research_write;
  logic [31:0] research_addr, research_wdata;
  ...
  m7_ibex_data_adapter u_data_adapt (
      ...
      .ibex_err_o(),
      .cpu_req_o(research_req),
      .cpu_addr_o(research_addr),
      .cpu_write_o(research_write),
      .cpu_wdata_o(research_wdata),
      .cpu_gnt_i(cpu_ic_gnt),
      .cpu_valid_i(cpu_ic_valid),
  ...
  simulator_ctrl #(.LogName("m7_ibex_composed.log")) u_sim_ctrl (
      .clk_i(clk_i), .rst_ni(rst_ni),
  ...
  iopmp u_iopmp (
      .clk(clk_i), .rst_n(rst_ni), .enable(iopmp_enable),
  ...
  m7_composed_harness u_harness (
      ...
      .prot_peek_addr(prot_peek_addr), .prot_mem_word0(prot_mem_word0),
      .prot_mem_changed(prot_mem_changed),
      .test_ready(test_ready_o), .go_pulse(),
      ...
      .iopmp_txn_valid(iopmp_txn_valid),
      .iopmp_txn_authorized(iopmp_txn_authorized),
