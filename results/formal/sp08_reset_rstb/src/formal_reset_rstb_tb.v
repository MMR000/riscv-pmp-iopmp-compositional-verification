// Reset formal harness RST-B: fail-closed IOPMP defaults until secure_ready.
`include "bus_pkg.vh"
`include "formal_helpers.vh"

module formal_reset_rstb_tb (
    input wire clk,
    input wire rst_n
);
    ...
    security_config u_sec (
        .clk(clk), .rst_n(rst_n && sec_rst_n && dut_rst_n),
    ...
    iopmp u_iopmp (
        .clk(clk), .rst_n(rst_n && iopmp_rst_n && dut_rst_n),
        .enable(iopmp_enable),
    ...
    bus_interconnect u_ic (
        .clk(clk), .rst_n(rst_n && ic_rst_n && dut_rst_n),
    ...
    wire dma_prot_commit = prot_mem_changed && u_iopmp.hold_write &&
                           `IN_PROT(u_iopmp.hold_addr) && iopmp_hold_authorized;

    always @(*) assume (!cfg_write);

    // SP-08 RST-B: no authorized protected DMA commit before secure_ready.
    always @(posedge clk) begin
        if (formal_warmup && !secure_ready && prot_mem_changed && `IN_PROT(prot_addr))
            assert(!dma_prot_commit);
    end

    always @(posedge clk) cover(formal_warmup && dma_active_before_secure && dma_req);
    always @(posedge clk) cover(formal_warmup && !secure_ready && iopmp_enable && !rule0_valid);
    always @(posedge clk) cover(formal_warmup && dma_to_prot && !secure_ready);
